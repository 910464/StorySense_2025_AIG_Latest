...
#         orchestrator.metrics_manager = metrics_manager_mock
#
#         return orchestrator
#
#     @pytest.fixture
...
#         assert 'embedding <= %s::vector' in call_args[0][0]  # Cosine distance operator
...
#         call_args = mock_cursor.execute.call_args
#         assert 'embedding <->; %s::vector' in call_args[0][0]  # L2 distance operator
...
#         call_args = mock_cursor.execute.call_args
#         assert 'embedding &lt;#&gt; %s::vector' in call_args[0][0]  # Inner product operator
#         assert 'ORDER BY embedding &lt;#&gt; %s::vector DESC' in call_args[0][0]  # Descending order
#
#         # Verify results
#         assert len(docs_with_score) == 1
...
#         assert mock_orchestrator.db.connect_with_retry.call_count == 3
#
...
#
#     def test_retrieval_context_threshold_filtering(self, retriever, mock_orchestrator):
#         """Test that results below threshold are logged correctly"""
#         # Setup mock cursor with results above and below threshold
#         mock_cursor = mock_orchestrator.db.cursor
#         mock_cursor.fetchall.return_value = [
#             ('High similarity doc', 0.85, {'source': 'doc1.pdf'}),
#             ('Medium similarity doc', 0.72, {'source': 'doc2.pdf'}),
#             ('Low similarity doc', 0.65, {'source': 'doc3.pdf'})  # Below threshold
#         ]
#
#         # Set threshold
...
#         )
#
#         # Verify all results are returned (filtering happens in SQL query)
#         assert len(docs_with_score) == 3
#
#         # Verify logging of results passing threshold
#         # (This would require mocking the logging module to verify)
#
...
#             assert call_args[0][1][3] == 0.56  # 0.7 * 0.8
#
...
#
#     @pytest.fixture
#     def retriever_with_errors(self, mock_orchestrator):
#         """Create retriever for error testing"""
#         return PGVectorRetriever(mock_orchestrator)
#
#     def test_retrieval_context_database_connection_error(self, retriever_with_errors, mock_orchestrator):
#         """Test handling of database connection errors"""
#         # Make reconnect_if_needed raise an error
#         mock_orchestrator.db.reconnect_if_needed.side_effect = Exception("Connection error")
#
#         # Call retrieval_context
...
#
#     def test_retrieval_context_embedding_generation_error(self, retriever_with_errors, mock_orchestrator):
#         """Test handling of embedding generation errors"""
#         # Make embed_query raise an error
#         mock_orchestrator.embeddings.embed_query.side_effect = Exception("Embedding error")
#
#         # Call retrieval_context
#         context, docs_with_score, docs_metadata, threshold = retriever_with_errors.retrieval_context(
#             query="test query",
...
#         mock_orchestrator.metrics_manager.record_error.assert_called()
#
#     def test_retrieval_context_sql_execution_error(self, retriever_with_errors, mock_orchestrator):
...
# tests/context_handler/context_storage_handler/test_pgvector_retriever.py
...
class TestPGVectorRetriever:
    ...
    def test_retrieval_context_logging(self, retriever, mock_orchestrator):
        ...
        with patch('logging.info') as mock_log:
            ...
            assert mock_log.call_count & gt;0
...
class TestPGVectorRetrieverEdgeCases:
    ...
    @pytest.fixture
    def mock_orchestrator_for_errors(self, metrics_manager_mock):
    ...
    def test_retrieval_context_metrics_recording_on_success(self, retriever_with_errors, mock_orchestrator_for_errors):
        ...
        assert call_args[1]['duration'] & gt;
        0
...
class TestPGVectorRetrieverSimilarityMetrics:
    ...
    @pytest.fixture
    def base_orchestrator(self, metrics_manager_mock):
